...
class FusionQwenConfig(Qwen2Config):
    model_type = "fusion_qwen"


class FusionQwenModel(FusionMetaModel, Qwen2Model):
    config_class = FusionQwenConfig

    def __init__(self, config: Qwen2Config):
        super(FusionQwenModel, self).__init__(config)


class FusionQwenForCausalLM(Qwen2ForCausalLM, FusionMetaForCausalLM):
    config_class = FusionQwenConfig

    def __init__(self, config):
        super(Qwen2ForCausalLM, self).__init__(config)
        config.rope_scaling = None
        self.model = FusionQwenModel(config)
...
AutoConfig.register("fusion_qwen", FusionQwenConfig)
AutoModelForCausalLM.register(FusionQwenConfig, FusionQwenForCausalLM)
