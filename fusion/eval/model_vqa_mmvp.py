...
from fusion.utils import disable_torch_init
from fusion.mm_utils import tokenizer_image_token, process_images, get_model_name_from_path
...
def eval_model(args):
    ...
    for i, line in enumerate(tqdm(questions)):
        idx = line["Index"]
        cur_prompt = line['Question'] + " " + line['Options']
        qs = cur_prompt

        if getattr(model.config, 'mm_use_im_start_end', True):
        ...
        instructs = tokenizer(cur_prompt.replace(DEFAULT_IMAGE_TOKEN, '').replace(DEFAULT_IM_START_TOKEN, '').replace(DEFAULT_IM_END_TOKEN, '').strip()).input_ids
        ...
        ans_file.write(json.dumps({"question_id": idx,
                                   "prompt": cur_prompt,
                                   "ground_truth": line["Correct Answer"], 
                                   "text": outputs,
                                   "answer_id": ans_id,
                                   "model_id": model_name,
                                   }) + "\n")
